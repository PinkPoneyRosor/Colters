    ...
    SubShader {
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float4 node_1 = tex2D(_Diffuse,TRANSFORM_TEX(node_579.rg, _Diffuse));
                clip(node_1.a - 0.5);
                i.normalDir = normalize(i.normalDir);
                float3x3 tangentTransform = float3x3( i.tangentDir, i.binormalDir, i.normalDir);
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
/////// Normals:
                float3 normalLocal = UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_579.rg, _Normal))).rgb;
...
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                float2 node_580 = i.uv0;
                float4 node_1 = tex2D(_Diffuse,TRANSFORM_TEX(node_580.rg, _Diffuse));
                clip(node_1.a - 0.5);
                i.normalDir = normalize(i.normalDir);
                float3x3 tangentTransform = float3x3( i.tangentDir, i.binormalDir, i.normalDir);
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
/////// Normals:
                float3 normalLocal = UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_580.rg, _Normal))).rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                float2 node_581 = i.uv0;
                float4 node_1 = tex2D(_Diffuse,TRANSFORM_TEX(node_581.rg, _Diffuse));
                clip(node_1.a - 0.5);
                i.normalDir = normalize(i.normalDir);
