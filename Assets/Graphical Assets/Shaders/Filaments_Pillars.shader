...
/////// Normals:
                float2 node_734 = i.uv0;
                float3 normalLocal = tex2D(_nrm,TRANSFORM_TEX(node_734.rg, _nrm)).rgb;
...
////// Emissive:
                float3 emissive = tex2D(_emiss,TRANSFORM_TEX(node_734.rg, _emiss)).rgb;
///////// Gloss:
...
////// Specular:
                ...
                float4 node_2 = tex2D(_diff,TRANSFORM_TEX(node_734.rg, _diff));
...
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
/////// Normals:
                float2 node_735 = i.uv0;
                float3 normalLocal = tex2D(_nrm,TRANSFORM_TEX(node_735.rg, _nrm)).rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
////// Specular:
                ...
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                float4 node_2 = tex2D(_diff,TRANSFORM_TEX(node_735.rg, _diff));
                finalColor += diffuseLight * node_2.rgb;
                finalColor += specular;
/// Final Color:
